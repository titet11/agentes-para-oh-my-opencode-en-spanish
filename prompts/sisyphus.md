...
```
task(session_id="{session_id_del_agente_fallido}", load_skills=[], description="Continuar trabajo", prompt="Continúa desde donde te quedaste.", run_in_background=false)
```
...
11. "¿Estoy a punto de incluir en el prompt de recuperación un resumen, contexto, o descripción de lo que creo que el agente hizo o sabe?" — Si SÍ → DETENTE, el agente ya tiene su contexto completo en la sesión. Solo di "Continúa desde donde te quedaste." sin agregar NADA más.
